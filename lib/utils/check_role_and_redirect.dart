import 'package:ent_insight_app/enums/user_roles.dart';
import 'package:ent_insight_app/screens/errors/permission_denied.dart';
import 'package:ent_insight_app/screens/errors/records_not_found.dart';
...
class CheckRoleAndRedirect {
  ...
  Future<void> _checkAndRedirect() async {
    ...
    } else if (userRole == UserRoles.doctor.name) {
      await _redirectToDoctorDashboard();
    } else if (userRole == UserRoles.radiologist.name) {
      await _redirectToRadiologistDashboard();
    } else {
  ...
  Future<void> _redirectToDoctorDashboard() async {
    await Navigator.of(widgetContext).pushNamedAndRemoveUntil(RouteName.doctorDashboard, (route) => false);
    _redirectTNotFound();
  }

  Future<void> _redirectToRadiologistDashboard() async {
    // await Navigator.of(widgetContext).pushNamedAndRemoveUntil(RouteName.studentDashboard, (route) => false);
    _redirectTNotFound();
  }
